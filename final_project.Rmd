...
Perform a prediction of new cases using decision tree model
```{r}
...
```
Perform a prediction of new cases using random forest model
```{r}
...
```{r}
linearfit <- lm(new_case ~ poly(Administered,4,raw=TRUE), data = trainset)
...
Prediction of new death after 2021-07-31 using decision tree model
```{r}
rpart_fit1 <- rpart(new_death_clas ~ new_case_clas+Administered, data = trainset)
rpart_preds1 <- predict(rpart_fit1, testset)
rpart_res1<-ifelse(rpart_preds1<1.5,1,ifelse(rpart_preds1>=2.5,3,2))

confusionMatrix(data = as.factor(rpart_res1), reference = as.factor(testset$new_death_clas),positive="1")
testset%>%
  ggplot()+geom_point(aes(date,rpart_preds1),color="red")+geom_point(aes(date,new_death_clas))
#rf_preds1
```
